...
const generateSessionId = () => "chat_" + Math.random().toString(36).substring(2, 10);
...
function App() {
  const [sessionId, setSessionId] = useState(() => localStorage.getItem('rag_session_id') || generateSessionId());
  
  // Language & Theme State
  const [lang, setLang] = useState(() => localStorage.getItem('rag_lang') || 'en');
  ...
  // 🔥 Traffic controller for toast notifications so they don't glitch when clicked fast
  ...
  useEffect(() => {
    ...
    if (messages.length > 0) {
      setChatSessions(prev => {
        ...
        if (!exists) {
          return [{ id: sessionId, title: messages[0].content.substring(0, 25) + "..." }, ...prev];
        } else if (exists.title === 'New Chat' || exists.title === 'Neuer Chat') {
          return prev.map(s => s.id === sessionId ? { ...s, title: messages[0].content.substring(0, 25) + "..." } : s);
        }
        return prev;
      });
    }
  ...
  const showToast = (message, type = 'success') => {
    ...
    // Clear the old timer if a new toast pops up immediately
  ...
  const handleDeleteChat = (e, idToDelete) => {
    e.stopPropagation();
    setChatSessions(prev => prev.filter(session => session.id !== idToDelete));
  ...
  const handleClearAllChats = () => {
    chatSessions.forEach(session => localStorage.removeItem(`rag_chat_${session.id}`));
...
const handleSendMessage = async (text) => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { role: 'assistant', content: t.connFail }]);
  ...
  return (
    <>
      ...
      <Show when="signed-in">
        <div className="flex h-screen w-screen bg-white dark:bg-[#0A0A0B] font-sans overflow-hidden transition-colors duration-300 relative selection:bg-violet-500/30">
          
          <Sidebar 
            activeTab={activeTab} setActiveTab={setActiveTab} 
            sessionId={sessionId} onNewChat={handleNewChat}
            chatSessions={chatSessions} onSwitchChat={handleSwitchChat}
            onDeleteChat={handleDeleteChat} onRenameChat={handleRenameChat}       
            onClearAllChats={handleClearAllChats} onExportChat={handleExportChat}
            lang={lang}
          />
          
          <main className="flex-1 flex flex-col h-full relative border-l border-zinc-200 dark:border-zinc-800/60 bg-white dark:bg-[#0A0A0B]">
            <header className="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800/60 bg-white/80 dark:bg-[#0A0A0B]/80 backdrop-blur-md flex justify-between items-center z-10 absolute top-0 w-full">
              <div>
                <h2 className="text-lg font-bold text-zinc-800 dark:text-zinc-100 tracking-tight">{t.missionControl}</h2>
                <div className="flex items-center gap-2 mt-0.5">
                  <span className="relative flex h-1.5 w-1.5">
                    <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                    <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-emerald-500"></span>
                  </span>
                  <p className="text-[11px] text-zinc-500 font-medium uppercase tracking-wider">{t.session}: {sessionId.substring(0, 9)}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => setIsDarkMode(!isDarkMode)} className="p-2 rounded-lg bg-zinc-100 dark:bg-zinc-900 hover:bg-zinc-200 dark:hover:bg-zinc-800 text-zinc-600 dark:text-zinc-400 transition-all border border-zinc-200 dark:border-zinc-800 shadow-sm">
            ...
            {activeTab === 'docs' && <div className="flex-1 overflow-hidden pt-20 h-full flex"><DocumentHub onReset={handleNewChat} /></div>}
            {activeTab === 'settings' && <div className="flex-1 overflow-hidden pt-20 h-full flex"><SettingsHub isDarkMode={isDarkMode} setIsDarkMode={setIsDarkMode} /></div>}
          </main>

          {/* 🔥 PREMIUM TOP-CENTERED TOAST NOTIFICATION */}
          <div className={`fixed top-8 left-1/2 -translate-x-1/2 z-[100] transition-all duration-400 ease-out ${toast.show ? 'translate-y-0 opacity-100 scale-100' : '-translate-y-4 opacity-0 scale-95 pointer-events-none'}`}>
            <div className={`flex items-center gap-3 px-6 py-3.5 rounded-full shadow-2xl border min-w-[300px] justify-center ${
              toast.type === 'error' 
